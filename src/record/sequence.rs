...
const SEQUENCE_MAPPER: &[u8; 16] = b"=AGSYKBDHVMRWCTN";
const SEQUENCE_MAPPER_ATCGN_ONLY: &[u8; 16] = b"=AGNNNNNNNNNNCTN";
...
/// nucleotide in the following order: `=AGSYKBDHVMRWCTN`.
#[derive(Clone)]
pub struct Sequence {
    raw: Vec<u8>,
    len: usize,
...
impl Sequence {
    ...
    pub fn extend_from_text<I: IntoIterator<Item = u8>>(
        ...
        nucleotides.into_iter().map(|nt| self.push(nt)).collect()
    }
    ...
    pub fn at(&self, index: usize) -> u8 {
        let nt = self.nucleotide_u4_at(index);
        SEQUENCE_MAPPER[nt as usize]
    }

    /// Returns the internal representation of a nucleotide at the position `index`, represented by a single byte, O(1).
    fn nucleotide_u4_at(&self, index: usize) -> u8 {
        assert!(
            index < self.len,
            "Index out of range ({} >= {})",
            index,
